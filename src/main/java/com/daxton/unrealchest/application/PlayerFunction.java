    ...
    public static void onCommand(Player player, String cmd) {
        try {
            ...
            if (commands.length >= 2) {
                ...
                // 指令格式不正確的情況
                UnrealChest.unrealCorePlugin.sendSystemLogger("指令格式錯誤!");
                UnrealChest.unrealCorePlugin.sendSystemLogger("command format error!");
            }
        } catch (Exception exception) {
            // 記錄異常日誌
            UnrealChest.unrealCorePlugin.sendSystemLogger("An exception occurred while processing the command: " + exception.getMessage());
        }
    }
